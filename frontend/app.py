...
with col2:
    ...
    if is_png:
        quality = st.slider(
            "Tingkat kompresi PNG (1 = cepat, 100 = maks kompresi)",
            min_value=1,
            max_value=100,
            value=50,
            help=(
                "Untuk PNG (lossless), nilai ini mengontrol tingkat kompresi "
                "bukan kualitas visual. Nilai lebih tinggi = file lebih kecil "
                "tetapi encoding lebih lambat."
            ),
        )
    else:
        quality = st.slider(
            ...
            max_value=100,
            value=80,
            help="Semakin tinggi = kualitas lebih baik, ukuran file lebih besar.",
        )
...
if enable_resize:
    max_dimension = st.number_input(
        "Dimensi Maksimal (px)", min_value=16, value=1080, step=1
    )
...
if st.button("Konversi", type="primary"):
    if not uploaded_files:
        ...
        with st.spinner("Mengonversi gambar..."):
            zip_result, stats = convert_images(
                uploaded_files,
                target_format,
                quality,
                max_dimension,
            ...
            if zip_result:
                ...
                if stats:
                    with st.expander("Lihat Statistik Konversi"):
                        ...
                        df["Original Size (KB)"] = (
                            (df["original_size"] / 1024).round(2).astype(str) + " KB"
                        )
                        df["Optimized Size (KB)"] = (
                            (df["compressed_size"] / 1024).round(2).astype(str) + " KB"
                        )
                        df["Size Reduction"] = (
